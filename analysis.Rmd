...
```{r}
p <- ggplot(data=by_inst_alg, aes(x=polynomials, y=time, color=algorithm, shape=reducer)) +
  geom_point() +
  scale_y_log10() +
  scale_x_log10() +
  xlab("log(polynomials)") +
  ylab("log(time)") +
  theme(legend.position=c(0.87, 0.32))
ggsave(paste0("./img/scatterpolytime.pdf"), height=4.5)
plot(p)

p <- ggplot(data=by_inst_alg, aes(x=degree, y=time, color=algorithm, shape=reducer)) +
  geom_point() +
  scale_y_log10() +
  scale_x_log10() +
  xlab("log(degree)") +
