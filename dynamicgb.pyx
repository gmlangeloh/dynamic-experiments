...
cpdef tuple dynamic_gb \
  (F, dmax=Infinity, strategy='sugar', \
   ...
   lscriterion = 'newton', print_criterion=False, taboo_tenure=0,
  ...
  """
    ...
    INPUTS:
      ...
      - `lscriterion` -- criterion used to eliminate leading monomials in local
        search. Either `newton`, `perry1` or `perry2`. newton corresponds to
        computing the vertices of an affine Newton polyhedron, perry1 is Perry's
        divisibility criterion without step c, perry 2 with step c
      - `print_criterion` -- whether to print data on the lscriterion
      - `taboo_tenure` -- Number of iterations a polynomial may remain unused in
        the localsearch algorithm. Set to 0 to disable taboo search. Only
        applicable for the localsearch algorithm.
  """
  random.seed(seed)
  set_random_seed(seed)
  statistics.set_options(print_results, return_stats, print_criterion,
                         print_coefficients)
  ...
  cdef LocalSearchState state = LocalSearchState(n, current_ordering, lscriterion,
                                                 heuristic, PR, taboo_tenure)
  ...
  statistics.update_coefficient_data(gb)
