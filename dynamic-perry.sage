...
#TODO separate this into more functions, optimize it, use static typing
@cython.profile(True)
cpdef list sensitivity(GLPKBackend lp, int n, int k):
  ...
  cdef list Bm1N, row, row_indices, row_coefs
  cdef Matrix_real_double_dense Bm1NT
  ...
  for i in basic:
    if i >= m:
      cB_l.append(float(lp.objective_coefficient(i - m)))
    else: #is slack variable
      cB_l.append(0.0)
  cdef Vector_real_double_dense cB = vector(cB_l)
  cdef list cN_l = []
  for i in nonbasic:
    if i >= m:
      cN_l.append(float(lp.objective_coefficient(i - m)))
    else:
      cN_l.append(0.0)
  cdef Vector_real_double_dense cN = vector(cN_l)
  cdef Vector_real_double_dense zN = Bm1NT * cB - cN

  #Compute delta zN
  cdef int change = randint(0, n-1)
  cdef Vector_integer_dense DcB = vector([ 0 ] * len(cB))
  cdef Vector_integer_dense DcN = vector([ 0 ] * len(cN))
  ...
  cdef Vector_real_double_dense DzN = Bm1NT * DcB - DcN
...
cpdef list choose_simplex_ordering(list G, list current_ordering, GLPKBackend lp, int iterations = 5):
  ...
  w = [ randint(1, 1000) for i in xrange(n) ]
  best_w = w
