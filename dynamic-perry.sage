...
cpdef tuple choose_cone_ordering(list G, list current_ordering, list constraints, MixedIntegerLinearProgram lp, set rejects, set bvs, int use_bvs, int use_dcs):
  ...
  cdef tuple result = choose_ordering_restricted(G, [ G[i].value().lm() for i in xrange(k-1)], k-1, current_ordering, lp, rejects, bvs, use_bvs, use_dcs)
  ...
  result = choose_ordering_restricted(G, [G[i].value().lm() for i in xrange(k-1)], k-1, current_ordering, lp, rejects, bvs, use_bvs, use_dcs)
...
cpdef tuple dynamic_gb(F, dmax=Infinity, strategy='normal', static=False, minimize_homogeneous=False, \
                       ...
                       reinsert=False, max_calls=Infinity, itmax=Infinity):
  r"""
  ...
  while len(P) != 0:
    ...
    if d < dmax: # don't go further than requested
      ...
      if r.value() != 0: # add to basis, choose new ordering, update pairs
        ...
        #Stop here if completed the requested number of iterations
