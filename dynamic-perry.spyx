...
cpdef tuple choose_cone_ordering(list G, list current_ordering, list constraints, MixedIntegerLinearProgram lp, set rejects, set bvs, int use_bvs, int use_dcs):
  ...
  #STEP 1: build the lists of useful and useless LTs
  cdef MPolynomialRing_libsingular R = G[0].value().parent()
  ...
  #also, this should probably be computed AFTER insertion
  ...
  #STEP 2: we insert the new polynomial with new LT using Caboara and Perry's algorithm.
  cdef int new_beginning = lp.number_of_constraints()
  cdef tuple result = choose_ordering_restricted(G, useful_LTs, k-1, current_ordering, lp, rejects, bvs, use_bvs, use_dcs)
  ...
  cdef MPolynomial_libsingular newLT = PR(G[k-1].value()).lm()
  ...
  print "suspicion 1", constraints, reinsert
  ...
  result = choose_ordering_restricted(G, useful_LTs + [newLT], k-1, current_ordering, lp, rejects, bvs, use_bvs, use_dcs)
...
cpdef tuple dynamic_gb(F, dmax=Infinity, strategy='normal', static=False, minimize_homogeneous=False, \
                       ...
                       reinsert=False, max_calls=Infinity):
  r"""
